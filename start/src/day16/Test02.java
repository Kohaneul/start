...
public class Test02 {
	...
	public Test02() {
		...
		//데이터 추가
		table.put("이름", "제니");
		//이처럼 Map 계열은 데이터를 입력할 떄
		//그 데이터를 구분할 키 값과 데이터를 한 쌍으로 만들어서 입력해야만 한다.
		table.put("나이", 27);
		...
		/*
		 	Map은 같은 Key값으로 데이터가 입력되면 먼저 데이터를 삭제한 후 새로운 데이터로 교체한다.
		 	즉 같은 Key 값은 한개의 데이터만 기억할 수 있다.
		 	==> 데이터는 중복될 수 있지만 키값은 중복되어서 사용될 수 없다.
		...
		System.out.println("나이 : "+table.get("나이"));
		System.out.println("전화 : "+table.get("전화"));
		System.out.println("name : "+table.get("name")); //데이터는 중복 가능하지만 key값은 중복 불가(중복된다면 value값이 교체됨)
		
		int len = table.size(); //저장된 데이터의 갯수를 반환해주는 함수
		...
		Set entry = table.entrySet();	//Map 저장된 데이터를 Set에 저장
		Iterator it3 = entry.iterator();
		
		while(it3.hasNext()) {
			Object o = it3.next(); //Object로 꺼내줌
