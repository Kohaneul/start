		...
		String[] flist = dir.list(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				 ...
				 * 이 함수는 자동 호출 함수이다.
				   ...
				   이 함수에서 true를 반환하면 해당목록을 결과에 포함시키고
				 ...
				 */
				boolean bool = false;
				if(name.endsWith("jpg")) {	//파일이름이 jpg로 끝나는 경우
					//이 경우는 결과 목록에 이 파일을 포함시킨다.
					bool = true;
				}//bool 변수는 초기값으로 이미 false로 셋팅되어있기 때문에 생략해도 무방하다.
				return bool;	
				//	==> return name.endsWith("jpg");
				//	==> 이 경우처럼 상속받아 오버라이드해야하는 함수의 내용이 극도로 짧은경우는
				//		이 예제처럼 무명내부클래스로 처리하는 것이 일반적.
			}
				});
