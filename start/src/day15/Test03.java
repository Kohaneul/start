...
/*
 	이름을 입력받아서 입력한 이름이 한글이름인지를 검사하는 프로그램을 작성하세요.
...
public class Test03 {
	...
	/*
	 	정규식 검사
	 		1. Pattern 클래스를 이용해서 검사하고 싶은 정규식 문법을 먼저 만든다.
	 		
	 		참고 ] 
	 			이 클래스는 생성자가 감춰져 있는 경우이므로
	 			소속 함수 중 속성이 static 이고 반환값 타입이 자기자신 Pattern 타입인 함수를 찾아서
	 			사용하면 된다.
	 		...
	 		2. 이 클래스에게 정규식 문법을 검사해달라고 요청한다.
	 			...
	 			따라서 검사 결고가 이 클래스의 객체 내부에 모든 정보가 기억되어 있다.
	 			검사결과를 꺼내오는 함수
